...
func TestWebhookSignature(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		receivedSig = r.Header.Get("X-Nyantenna-Signature")
...
func TestWebhookNoSignatureWhenSecretEmpty(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		hasSigHeader = r.Header.Get("X-Nyantenna-Signature") != ""
		w.WriteHeader(http.StatusOK)
	}))
	defer srv.Close()
	...
	if hasSigHeader {
		t.Error("secret 为空时不应携带 X-Nyantenna-Signature header")
	}
}
...
func TestWebhookCustomHeadersCannotOverrideProtected(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		gotSig = r.Header.Get("X-Nyantenna-Signature")
	...
	ch, err := NewWebhookChannel(config.WebhookConfig{
		...
		Headers: map[string]string{
			...
			"X-Nyantenna-Signature": "sha256=forged", // 尝试伪造签名
			"":                      "ignored",       // 空 key 应被丢弃
		},
	})
	if err != nil {

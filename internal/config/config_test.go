...
`)
	...
	err := AddDeviceInFile(path, DeviceConfig{
		...
		QMIProxyExecutable: "/opt/nyantenna/bin/qmi-proxy",
	})
	if err != nil {
	...
	if dev.QMIProxyExecutable != "/opt/nyantenna/bin/qmi-proxy" {
		t.Fatalf("QMIProxyExecutable=%q, want /opt/nyantenna/bin/qmi-proxy", dev.QMIProxyExecutable)
	}
}

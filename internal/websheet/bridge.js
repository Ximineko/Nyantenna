(function(){
  ...
  const shellBroadcastName = "nyantenna-websheet";
  const shellStorageKey = "nyantenna-websheet-complete";
    ...
    try {
      ...
      if (originalOpen) {
        window.XMLHttpRequest.prototype.open = function(method, url) {
          ...
          this.__nyantennaMethod = String(method || "GET").toUpperCase();
          this.__nyantennaURL = String(url || "");
        ...
        window.XMLHttpRequest.prototype.send = function() {
          try {
            this.addEventListener("load", function() {
              inspectATTAddressResponse(this.__nyantennaMethod || "GET", this.__nyantennaURL || "", this.responseText || "");
            });
          } catch (_) {}
  ...
  function shellMessage(payload) {
    return {type:"nyantenna-websheet-callback", token:websheetToken || "", callback: payload};
  }

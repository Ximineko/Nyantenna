...
// qmiModemAdapter 将后端适配为 vowifi.Modem + simauth.ATModem 接口，
// 使 VoWiFi 在没有 AT 串口的形态下也能完成 EAP-AKA SIM 鉴权。
//
// 名字沿用历史，但实际服务于所有非 AT 后端：QMI、MBIM，以及只有读卡器的 PC/SC。
// 它只依赖 DeviceBackend 的逻辑通道与 APDU 能力，与底层是模组还是读卡器无关。
//

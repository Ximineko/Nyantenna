...
func TestVoWiFiHostImportsExternalRuntimehostOnly(t *testing.T) {
	...
	for _, root := range scanRoots {
		...
		err := filepath.WalkDir(base, func(path string, d os.DirEntry, err error) error {
			...
			for _, spec := range file.Imports {
				...
				if importPath == "github.com/ximineko/"+"nyantenna/internal/vowifi" ||
					strings.HasPrefix(importPath, "github.com/ximineko/"+"nyantenna/internal/vowifi/") {
	...
	if len(offenders) > 0 {
		t.Fatalf("Nyantenna must import only approved vowifi-go runtimehost public packages:\n%s", strings.Join(offenders, "\n"))
	}
}

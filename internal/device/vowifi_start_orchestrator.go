...
import (
	...
	innersim "github.com/ximineko/nyantenna/internal/sim"
	"github.com/ximineko/nyantenna/internal/upstreamproxy"
	"github.com/ximineko/nyantenna/internal/vowifihost"
	"github.com/ximineko/nyantenna/pkg/logger"
	"github.com/ximineko/nyantenna/pkg/mbim"
)
...
func (p *Pool) prepareVoWiFiStartContext(deviceID, traceID, runtimeEPDGOverride string) (voWiFiStartContext, error) {
	...
	if _, ok := modemIface.(*qmiModemAdapter); ok {
		// 该适配器服务于所有非 AT 后端（QMI / MBIM / PC/SC），日志按实际模式写，
		// 否则 PC/SC 设备会被误记成走了 QMI。
		logger.Debug("VoWiFi 通过后端逻辑通道鉴权",
	...
	logger.Debug("VoWiFi 启动前读取当前设备身份",
		"trace_id", traceID,
		"device", deviceID,
	...
	if strings.EqualFold(workerAKAProviderInput{worker: w}.BackendMode(), backend.BackendMBIM) {
		logger.Debug("VoWiFi 使用 MBIM Auth(AKA) 鉴权", "trace_id", traceID, "device", deviceID)
	} else {
		logger.Debug("VoWiFi 使用 APDU(AKA) 鉴权", "trace_id", traceID, "device", deviceID)
	}
	...
	if nc := w.NetworkController(); nc != nil {
		...
		logger.Debug("VoWiFi 启用中，停止网络功能", "trace_id", traceID, "device", deviceID)
	...
	if strings.EqualFold(w.Backend.Mode(), backend.BackendMBIM) {
		logger.Debug("MBIM 后端不支持真正的低功耗模式",
			"trace_id", traceID, "device", deviceID)
	} else if alreadyInFlight {
		logger.Debug("设备已处于飞行模式，跳过冗余的飞行模式切换",
			"trace_id", traceID, "device", deviceID, "backend", w.Backend.Mode())
	} else {
		logger.Debug("进入飞行模式以禁用原生 IMS 注册",
			"trace_id", traceID, "device", deviceID, "backend", w.Backend.Mode())

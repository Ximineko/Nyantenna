...
import (
	...
	"github.com/ximineko/nyantenna/internal/cardpolicy"
	...
	mbimcore "github.com/ximineko/nyantenna/internal/mbim"
	"github.com/ximineko/nyantenna/internal/modem"
	"github.com/ximineko/nyantenna/internal/proxy/server"
	qmicore "github.com/ximineko/nyantenna/internal/qmi"
...
type Worker struct {
	...
	// pcscCard 仅 device_backend=pcsc 时非空；此类设备没有 Modem/QMICore/MBIMCore。
	pcscCard    *pcsc.Card
...
func (p *Pool) rescanAndReconnect(opts rescanReconnectOptions) error {
	...
	// PC/SC 设备没有 USB 模组可比对，放进来只会被判成 Offline 然后被误拆
	managed := excludePCSCDevices(config.ListDevices())

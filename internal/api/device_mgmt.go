...
import (
	...
	"github.com/ximineko/nyantenna/internal/e911"
	"github.com/ximineko/nyantenna/internal/esim"
	"github.com/ximineko/nyantenna/internal/modem"
	proxytraffic "github.com/ximineko/nyantenna/internal/proxy/traffic"
...
type deviceMgmtListItem struct {
	...
	NetworkConnected       bool                `json:"network_connected"`
	RegistrationStateLabel string              `json:"registration_state_label"`
	// BackendMode 让设备页能区分读卡器与模组：两者能力集差别很大
	// （PC/SC 没有射频、没有数据连接、没有 AT/选网/eSIM）。
	BackendMode string `json:"backend_mode,omitempty"`
}
...
func (s *Server) handleDeviceMgmtDiscovered(c *gin.Context) {
	...
	if err != nil {
		// 模组枚举失败不代表没有可添加设备：PC/SC 读卡器根本不经 USB 模组发现，
		// 一台只插读卡器、没有模组的机器上，走到这里才是常态。
		readers, pcscErr := discoverPCSCReaders(managed)
		respondDiscovered(c, readers, pcscErr)
		return
	}
	...
	pcscReaders, pcscErr := discoverPCSCReaders(managed)
	out = append(out, pcscReaders...)

	respondDiscovered(c, out, pcscErr)
}

// respondDiscovered 统一输出发现结果，保证 devices 恒为数组而非 null。
func respondDiscovered(c *gin.Context, devices []discoveredDevice, pcscErr string) {
	if devices == nil {
		devices = []discoveredDevice{}
	}
	resp := gin.H{"devices": devices}
	if pcscErr != "" {
		resp["pcsc_error"] = pcscErr
	}
	c.JSON(http.StatusOK, resp)
}

var listPCSCReadersFn = pcsc.ListReaders

// discoverPCSCReaders 把系统上的 PC/SC 读卡器也列为可添加设备。
//
// 读卡器不经 USB 模组枚举，也探不到 IMEI（没有基带），因此固定标记为
// mode=pcsc、network_capable=false；IMEI 由用户在添加时手工填写。
//
// 第二个返回值是给用户看的错误说明：仅当本二进制"带 PC/SC 支持但枚举失败"时非空。
// 未启用构建标签属于预期情况，静默跳过——否则每个普通用户都会看到一条无关告警。
func discoverPCSCReaders(managed []config.DeviceConfig) ([]discoveredDevice, string) {
	readers, err := listPCSCReadersFn()
	if err != nil {
		if errors.Is(err, pcsc.ErrPCSCUnavailable) {
			logger.Debug("本构建未启用 PC/SC 支持，跳过读卡器枚举")
			return nil, ""
		}
		// 到这里说明二进制是带 pcsc 标签构建的，用户多半确实想用读卡器，
		// 却因 pcscd 未运行/缺 CCID 驱动等原因失败——必须让他看见，
		// 而不是在界面上留下一句"暂无可添加设备"。
		logger.Warn("PC/SC 读卡器枚举失败", "err", err)
		return nil, fmt.Sprintf("PC/SC 读卡器枚举失败: %v（请确认 pcscd 已运行且已安装 CCID 驱动）", err)
	}

	claimed := make(map[string]string, len(managed))
	for _, cfg := range managed {
		if reader := strings.TrimSpace(cfg.PCSCReader); reader != "" {
			claimed[reader] = cfg.ID
		}
	}

	out := make([]discoveredDevice, 0, len(readers))
	for _, name := range readers {
		configuredID := claimed[name]
		out = append(out, discoveredDevice{
			DiscoveryKey: "pcsc:" + name,
			ControlPath:  name,
			DriverName:   "PC/SC",
			Mode:         "pcsc",
	...
	if len(out) == 0 {
		return nil, "已启用 PC/SC 支持，但系统上没有检测到读卡器（请确认读卡器已插好、pcscd 已运行）"
	}
	return out, ""
}
...
func validateDeviceBackendConfig(cfg config.DeviceConfig) error {
	...
	case "", "at", "qmi", "mbim":
		// 合法值
	case "pcsc":
		// PC/SC 读卡器没有基带，IMEI 读不到也无从生成，
		// 而 IKEv2 IDi 与 IMS 头部都要用，因此必须手工填写。
		if strings.TrimSpace(cfg.ModemIMEI) == "" {
			return fmt.Errorf("PC/SC 设备必须填写 IMEI（读卡器无基带，无法自动获取）")
		}
	default:

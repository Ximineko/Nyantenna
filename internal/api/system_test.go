...
// /etc/nyantenna/config.yaml，与进程工作目录无关）永远指向一个不存在的目录，
// 导致真实配置文件从未被清理。
func TestResolveUninstallTargetsUsesActualConfigPath(t *testing.T) {
	dataDir, configFile := resolveUninstallTargets("/etc/nyantenna/config.yaml")

	if dataDir != "data" {
		t.Fatalf("dataDir = %q, want %q", dataDir, "data")
	}
	if configFile != "/etc/nyantenna/config.yaml" {
		t.Fatalf("configFile = %q, want %q", configFile, "/etc/nyantenna/config.yaml")
	}
}
...
func TestDetectServiceStopCommandsPrefersOpenWrtInitScript(t *testing.T) {
	statFile := func(path string) bool { return path == "/etc/init.d/nyantenna" }
	...
	if cmds[0][0] != "/etc/init.d/nyantenna" || cmds[0][1] != "disable" {
		t.Fatalf("cmds[0] = %v, want [/etc/init.d/nyantenna disable]", cmds[0])
	}
	if cmds[1][0] != "/etc/init.d/nyantenna" || cmds[1][1] != "stop" {
		t.Fatalf("cmds[1] = %v, want [/etc/init.d/nyantenna stop]", cmds[1])
	}
}
...
func TestDetectServiceStopCommandsFallsBackToSystemd(t *testing.T) {
	...
	want := []string{"systemctl", "disable", "--now", "nyantenna"}

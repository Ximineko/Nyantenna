...
// OpenWrt 部署通过 -c 显式传入 /etc/nyantenna/config.yaml，与工作目录无关，
...
func detectServiceStopCommands(lookPath func(string) (string, error), statFile func(string) bool) [][]string {
	var cmds [][]string
	if statFile("/etc/init.d/nyantenna") {
		cmds = append(cmds, []string{"/etc/init.d/nyantenna", "disable"})
		cmds = append(cmds, []string{"/etc/init.d/nyantenna", "stop"})
		return cmds
	}

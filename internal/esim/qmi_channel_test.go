...
func TestNewQMIUIMTransportWithOptionsStoresClientOptions(t *testing.T) {
	transport := NewQMIUIMTransportWithOptions("/dev/cdc-wdm0", qmiq.ClientOptions{
		...
		ProxyExecutable: "/opt/nyantenna/bin/qmi-proxy",
	})
	...
	if transport.clientOptions.ProxyExecutable != "/opt/nyantenna/bin/qmi-proxy" {
		t.Fatalf("ProxyExecutable=%q, want /opt/nyantenna/bin/qmi-proxy", transport.clientOptions.ProxyExecutable)
	}

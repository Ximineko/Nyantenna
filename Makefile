...
DIST_DIR ?= dist
MAIN_PACKAGE ?= ./cmd/nyantenna

LDFLAGS = -s -w -X 'github.com/ximineko/nyantenna/internal/global.Version=$(VERSION)' -X 'github.com/ximineko/nyantenna/internal/global.BuildTime=$(BUILD_TIME)'
...
frontend-dist:
	npm install --prefix web-next --no-audit --no-fund
	npm run generate --prefix web-next
	rm -rf internal/web/dist
	mkdir -p internal/web
	cp -R web-next/.output/public internal/web/dist

...
import (
	...
	"github.com/ximineko/nyantenna/internal/api"
	...
	"github.com/ximineko/nyantenna/internal/notify"
	proxyserver "github.com/ximineko/nyantenna/internal/proxy/server"
	"github.com/ximineko/nyantenna/internal/proxy/traffic"
	"github.com/ximineko/nyantenna/internal/upstreamproxy"

	"github.com/ximineko/nyantenna/internal/web"
...
func main() {
	...
	logger.Info("Nyantenna 模组管理器启动中...")
...
// resolveDatabasePath 返回数据库文件路径。
//
// 项目更名前的数据库叫 data/vohive.db。新名字下若还没有数据库、而旧文件
// 仍在，就继续沿用旧文件：否则升级后会静默建一个空库，历史短信与设置
// 全部读不到，而用户看到的只是"数据没了"。
func resolveDatabasePath() string {
	const current = "data/nyantenna.db"
	const legacy = "data/vohive.db"
	if _, err := os.Stat(current); err == nil {
		return current
	}
	if _, err := os.Stat(legacy); err == nil {
		logger.Warn("沿用更名前的数据库文件", "path", legacy, "current", current)
		return legacy
	}
	return current
}
